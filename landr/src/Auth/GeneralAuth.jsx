...
https://landrentals.azurewebsites.net/api/Authentications/CreateUser
        try {
...
console.log(response)
            if (!response.ok) {
                const text = await response.json() // fallback in case it's not JSON
                console.error("Raw response error:", text.message);
