...
const EditPropertyForm = ({ property, onClose, onUpdate }) => {
  const [formData, setFormData] = useState({
    ...
    maritalStatus: property.maritalStatus || '',
    ageRange: property.ageRange || '',
    additionalRequirements: property.additionalRequirements || ''
  });

  const [mediaPreviews, setMediaPreviews] = useState(property.images.map(img => ({
    url: img.url,
    type: img.type || 'image'
  })));
              ...
              {activeSection !== 'basic' && (
                <button
                  ...
                  onClick={() => setActiveSection(prev => {
                    const sections = ['basic', 'amenities', 'requirements', 'images'];
